...
if __name__ == '__main__':
    ...
    QUERY_LIST = set(temp_list)
        # print(QUERY_LIST)
    # init the word frequency model with a simple list of all possible words

    # *****************************************************************************

    paths = os.path.abspath(os.path.join(os.getcwd(), "Dictionary.txt"))
    path = open(paths, 'r')
    content = path.read()
    path.close()
    DICT  = content.split()
    ...
    word_model = train(file('Dictionary.txt').read())
    ...
    for term in QUERY_LIST:
        ...
        if term not in DICT:
            ...
            CORRECT_MAP[term] = [(x, word_model[x]) for x in short_circuit_result]
            # if VERBOSE:
                # print best(word, short_circuit_result, word_model)
    print(CORRECT_MAP)
    # **********************************************************************
    output = open("Replacements.txt", 'w')
    for key, value in CORRECT_MAP.items():
        line = key
        max = 0
        replacement = ''
        if len(value) == 1:
