...
def populate_dicts():
    paths = os.path.abspath(os.path.join(os.getcwd(), "../../Task 1/Step 2-Index Generation"))
...
def calculate_score(output,query):
    ...
    for key,value in bm_score[:100]:
        output.write("\n"+qid+" Q0 "+key +" "+str(i)+" "+str(value)+" BM25NoStopNoStemPRF")
        i+=1


# creating inverted index into a dictionary
def create_index_dict():
    paths = os.path.abspath(os.path.join(os.getcwd(), "../../Task 1"))
...
def calculate_scores():
    ...
    paths = os.path.abspath(os.path.join(os.getcwd(), "../Step 1 - Query expansion"))
    path = open(os.path.join(paths,"expandedQueries.txt"),'r',encoding='utf-8')
    content = path.read()
    path.close()
    queries = content.split("\n")
    queries = [q for q in queries if q!=""]
    output = open("BM25Scores_NoRelevance_PRF.txt",'w',encoding='utf=8')
